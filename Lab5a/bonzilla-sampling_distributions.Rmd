...
1.  Describe this population distribution.  
  ...
  <style>
div.blue { background-color:#EFC1FB; border-radius: 5px; padding: 20px;}
</style>
<div class = "blue">
<font size="4">
  The distribution is right-skewed</font> 
...
<font size="4">
  The median and mean of the sample are greater than the population distribution (1442, 1500 & 1547,1573 for population and sample respectively). However, the range of the sample (810-3086) is not as wide as the population's (334-5642). The envelope of the two distributions are quite different. The population data has a distinct right-skewed shape. The sample distribution has a similar profile, but not as well defined. (</font> 
...
<font size="4">
<br>The mean of the samp1 is greater than the mean of samp2. However, the two values are relatively close.  
<br>If we were to take the sample means of two more random draws of size 100 and 1000, the sample of 1000 would provide a more accurate estimate of the population mean.
<br></font> 
...
<font size="4">
<br>From the output of the code above, there are 5000 elements in sample_means50. The distribution has a symmetrical, bell-shape and appears to be a normal distribution because the mean and median are so similar (1500 & 1498 respectivey).  
<br>If we were to collect 50,000 sample of size 50  and collect the means, we would be able to show the distribution with much smaller bins (and it would look smoother), but the profile of the distribution would look very similar. But why not just give it a try:  </font>
...
hist(sample_means50k, breaks = 100)
```
...
<font size="4">
<br>`sample_means_small` holds 100 elements. Each element is the mean of a different 100 member sample drawn from `area`; one sample for every iteration of the for loop.
<br></font> 
...
<div class = "blue">
<font size="4">
  The sample center remains the same with larger sample size. However, the spread of the distribution get's more narrow with increasing sample size</font> 
...
<font size="4">
  <br>The best point estimate of the population mean from the sample is the sample mean: </font> 
```{r}
mean( samp_price50 )
```
...
<font size="4">
  <br>The distribution is symmetric & bell-curved. Based on the distribution, the mean home price is approximately 182000</font> 
```{r}
cat('The mean home price based on the distribution of means:', mean(sample_means50))
cat('\nThe mean home price based on the population:', mean(price))
```
...
hist(sample_means50, breaks = 50, xlim = xlimits)
hist(sample_means150, breaks = 50, xlim = xlimits)
```

<font size="4">
  <br>The sampling distribution for `sample_means150` is symmetric & bell-shaped. The shape is more concentrated about the mean in comparison to the `sample_means50` distribution. 
  <br>Based on the plot of `sample_means150`, the mean home price is approximately 810,000</font> 
...
<div class = "blue">
<font size="4">
  <br>The sampling distribution for `sample_means150` has a smaller spread than the distribution for `sample_means50`
  <br>A distribution with a smaller spread is prefered for making estimates loser to the true value.</font> 
