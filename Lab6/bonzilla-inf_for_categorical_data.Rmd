...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    There are several key findings reported in the first paragraph such as 'the global average (who claim to be religious) is 59%'. These percentages appear to be sample statistics because the text states that the numbers are derived from polling data.
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    To generalize the finding to the global human population we must assume that the sampling method is random and the samples are independent. This sounds like a reasonable assumption because the data was collected from ~50k individuals, ~1000 from each country across 57 countries, therefore it seems reasonable to assume that the people selected were not connected.
...
3.  What does each row of Table 6 correspond to? What does each row of 
    ...
    Each row of Table 6 corresponds to the data for a given country for the sample size and the percent respondents for each category to the question, "Irrespective of whether you attend a place of worship or not, would you say you are a religious person, not a religious person or a convinced athiest?"  
    Each row of `atheism` corresponds to a single person's response and gives the nationality, response category & year.
...
4.  Using the command below, create a new dataframe called `us12` that contains
    ...
    The proportion calculated from `atheism` is 4.99 which rounds to 5% and is in agreement with the statistic in Table 6 for the percentage of United States self-identified atheists in 2012.

```{r us-atheism, eval=TRUE}
us12 <- subset(atheism, nationality == "United States" & year == "2012")
#head(us12)
us12A <- subset( us12, response == "atheist")
pAtheist <- nrow(us12A)/nrow(us12)
cat('Proportion of 2012 USA atheist responses =', pAtheist*100 )
```
...
5.  Write out the conditions for inference to construct a 95% confidence
    ...
    * independent observations: it is not explicitly stated, but I assume the sample is random, so one respondent's response is independent of another. 
    * at least 10 successes and 10 failures: #US2012 non atheists (~(1000-50 )) is > 10 and #US2012 atheists (~50) is > 10.  
    I am confident that both of these conditions are met.
...
SE <- 0.0069
z_conflev <- 1.96
marginError <- z_conflev * SE
cat('Margin of error for the proportion of US2012 atheists = ', marginError)
```
...
SE <- 0.0132
marginError <- z_conflev * SE
cat('Margin of error for the proportion of Japan 2012 atheists = ', marginError)
```
The conditions for inference are met because there are >10 successes & failures & the sample is random & independent.

```{r}
#MOLDOVA
m12 <- subset(atheism, nationality == "Moldova" & year == "2012")
inference(m12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
SE <- 0.0066
marginError <- z_conflev * SE
cat('Margin of error for the proportion of Moldova 2012 atheists = ', marginError)
```
The conditions for inference are met because there are >10 successes & failures & the sample is random & independent.
...
The margin of error is greatest when the population proportion is 0.5 and decreases to 0 for proportions = 0 an 1.0.
...
sd( p_hats )
```
    The shape of the sampling distribution is a symmetric, bell-shaped or normal curve. It's mean is centered on ~0.1 with a standard deviation (spread) of 0.0093
...
p <- 0.1
n <- 1040
p1_n1040hats <- rep(0, 5000)
for(i in 1:5000){
  samp <- sample(c("atheist", "non_atheist"), n, replace = TRUE, prob = c(p, 1-p))
  p1_n1040hats[i] <- sum(samp == "atheist")/n
}

p <- 0.1
n <- 400
p1_n400hats <- rep(0, 5000)
for(i in 1:5000){
  samp <- sample(c("atheist", "non_atheist"), n, replace = TRUE, prob = c(p, 1-p))
  p1_n400hats[i] <- sum(samp == "atheist")/n
}

p <- 0.2
n <- 1040
p2_n1040hats <- rep(0, 5000)
for(i in 1:5000){
  samp <- sample(c("atheist", "non_atheist"), n, replace = TRUE, prob = c(p, 1-p))
  p2_n1040hats[i] <- sum(samp == "atheist")/n
}

p <- 0.2
n <- 400
p2_n400hats <- rep(0, 5000)
for(i in 1:5000){
  samp <- sample(c("atheist", "non_atheist"), n, replace = TRUE, prob = c(p, 1-p))
  p2_n400hats[i] <- sum(samp == "atheist")/n
}

hist(p1_n1040hats, main = "p = 0.1, n = 1040", xlim = c(0, 0.3))
hist(p1_n400hats, main = "p = 0.1, n = 400", xlim = c(0, 0.3))
hist(p2_n1040hats, main = "p = 0.2, n = 1040", xlim = c(0, 0.3))
hist(p2_n400hats, main = "p = 0.2, n = 400", xlim = c(0, 0.3))

```
    
    * plot p=0.1, n= 400 is symmetric normal shaped with a mean centered on ~0.1. The spread is wider than the plot with the same p but more samples
    * plot p = 0.2, n=1040 is symmetric & normal shaped with a center ~0.2.
    * plot p=0.2, n=400 is symmetric & normal shaped with a center on ~0.2. The spread is wider than the plot with the same p but more samples.
    
    Based on these plots, The higher the number of samples (n), the narrower the sampling distribution. Changing values of p shifts the center of the distribution.
...
11. If you refer to Table 6, you'll find that Australia has a sample 
    ...
    Check to see if the conditions of inference are met:
    * we can continue the assumption that the samples are random & independent for both Ecuador and Australia.
    * For Australia #success = 0.1*1040 = 104; #fail = 1040-104. Both success & failures are > 10.
    * for Ecuador #success = 0.02*400 = 8; #fail= 392. The successes < 10
    Therefor, it is sensible to proceed with inference for Australia, but not in the case of Ecuador.
...
```{r}
#Create a new data set for respondents from Spain.
Spain2012 <- subset(atheism, nationality == "Spain" & year == "2012")
Spain2005 <- subset(atheism, nationality == "Spain" & year == "2005")

#Form confidence intervals for the true proportion of athiests in both years, and determine whether they overlap.
print('Spain 2012')
...
```
    ...
    There is overlap in the confidence intervals for Spain's 2005 and 2012 atheism index data, therefore we do not have convincing evidence that there has been change to the index between these two years.
...
```
    ...
    There is no overlap between the confidence intervals for the United States's 2005 and 2012 atheism index data, therefore we have convincing evidence that there has been change to the index.
...
-   If in fact there has been no change in the atheism index in the countries 
    ...
    *Hint:* Look in the textbook index under Type 1 error.
    ...
    If there had in fact been no change in any of the 39 countries listed in table 4 at a significance level of 0.05 we would expect to see change in 0.05*39 = 1.95, or 1-2 countries simply by chance.
...
-   Suppose you're hired by the local government to estimate the proportion of 
    ...
    *Hint:* Refer to your plot of the relationship between $p$ and margin of 
    ...
    To minimize tthe margin of error below 1% we would need tosample a relatively large proportion of the population (a large 'n'). the minimum 'n' that will meet our criteria can be found as follows:
```{r}
z <- 1.96 #for 95% confidence
p <- 0.5 #because we have no idea what to expect for p (assume worst case scenario)
m <- 0.01 #max margin of error
n <- z^2*p*(1-p)/m^2
n
```
