...
The sample distribution appears to be right-skewed; it has a tail to the right due to some high values for larger homes. The typical, or median size of a house in Ames, Iowa is 1382


2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

Another student's distribution would not be identical to mine, because it is a random draw of 60 samples from the population; another student would be drawing different observations, so the distribution would reflect variations. However, it would look similar (e.g. having values in the same range)
...
The observtions need to be independent and the success-failure condition must be met. np and n(1-p) must both be >=10
...
"95% confidence" means that we can be 95% confident that the interval captures the population proportion. In other words, if we have 100 different samples, then compute 95% confidence intervals for the samples, 95% os these confidence intervals would then hold the population mean.
...
Yes, the true population average (1499.69) is within my confidence interval(1365.868 1630.966). There is a 95% chance that my neighbor's confidence interval captures the true population average.
...
95% of the confidence intervals should capture the true population mean. This is because there is a standard error associated with the sample distribution; the natural variation from random sampling can result in a mean outside of the 95% interval.
...
Proportion of confidence intervals that include the true pop. mean: 46/50 = 0.92 
The proportion is not exactly equal to the confidence level. This is because only 50 samples were drawn. The higher the number of samples that are drawn, the closer the proportion with approach the exact confidence level.

-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
    
For a 99% Confidence Level, the critical value (Z-score) is 2.575 .   
...
Proportion of confidence intervals that include the true pop. mean: 49/50 = 0.98
This proportion approximates the confidence interval of 0.99; it is not exact because of the low number of samples
