...
```
    ...
    There is an upward trend in the data as a function of at_bats with no obvious sign of curvature; this suggests a linear model might be appropriate. However, the data is highly variale. Therefore I would not be comforatble predicting the runs until the linear model is evaluated further...
...
2.  Looking at your plot from the previous exercise, describe the relationship 
    ...
    There is a positive correlation between `runs` and `at_bats` as the form and direction of the points are sloped upwards across to the right of the x axis. However, the relationship is not very strong, because the spread is high (points are dispersed not tightly packed in a line).
...
# Note that this chunk will only run in interactive mode
...
3.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    I made 5 attempts:
```{r}
attempts <- c( 239656.5, 159988.4, 152182.3, 153559.8, 172904.1 )
min( attempts )
```
...
\]
    ...
    The slope is positive; this tells us that there is a positive relationship between a teams success and it's homeruns. A team with more homeruns will have more runs than a team with less homeruns.
...
```{r}
ab <- 5578
y <- -2789.2429 + 0.6305 * ab
cat( 'predicted runs with 5578 at-bats:', y )
```
    
    
    The predicted runs would be 727 runs. However, the observed runs were 713. Therefore, this is an overestimate of 14 runs.
...
6.  Is there any apparent pattern in the residuals plot? What does this indicate
    ...
    There is no apparent pattern in the residuals. This indicates that the residuals are scattered randomly.
...
7.  Based on the histogram and the normal probability plot, does the nearly 
    ...
    Yes, the histogram appears to have a nearly normal distribution and the QQ plot roughly follows a straight line suggesting a linear relationship.
...
8.  Based on the plot in (1), does the constant variability condition appear to 
    ...
    Yes, There is no apparent increase in variability as the data plots across the x-axis.
...
```
    ...
    At first glance, yes, there does seem to be a linear relationship.
...
-   How does this relationship compare to the relationship between `runs` and 
    ...
    Like the relationship between `runs` and `at_bats`, the relationship between `runs` and `hits` is a positive relationship; both slopes are positive and the data points are shaped upwards and to the right in the scatter plots. The R^2 for `runs` and `hits` is 0.6292 indicating that a linear model accounts for ~63% of the variance in the data. This is much greater than the `at_bats` R^2 (0.3729) which suggests that the relationship of `runs` and `hits` is better described by a linear model than relationship of `runs` and `at_bats`.
...
```
    ...
    The variable `bat_avg` best predicts the variable `runs`. The higher a teams `bat_avg`, the more runs it will likely have as shown in the scatter plot above. The R^2 for the `bat_avg` (0.6438) was highest out of all of the other 'traditional' variables which suggests that it has the strongest linear relationship to the `runs` features and can account for 64.4% of the variability.
...
```
    ...
    All three of the 'new' features outperformed the 'traditional' features at predicting `runs`, because all three of the new features had much higher R^2 values compared to the traditional measures suggesting a much stronger linear relationship for the new features. the feature `obs` performed the best of all 10 variables with an R^2 of 0.9326 (accounting for 93.26% of the variability in the data) whereas the best of the traditional measures could only account for 64.4%

-   Check the model diagnostics for the regression model with the variable you 
    decided was the best predictor for runs.
    
    **Linearity** there is no apparent pattern in the residual which suggests a linear model is appropriate
```{r}
plot( m_newobs$residuals ~ mlb11$new_obs )
abline(h=0, lty=3)
```
    
    **Nearly Normal Residuals** the histogram of the residuals approximates a noral distribution
```{r}
hist( m_newobs$residuals )
```
    
    **Constant Variability** Based on the residual plot, we don't see an obvious change in the amplitude of the residuals as a function of 'new_obs'.
    
    All three conditions are met, therefore this model passes it's diagnostics health check.
