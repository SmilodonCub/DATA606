...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    A streak length of 1 means that there was one hit followed by 1 miss.  
    A streak length of 0 means that there was no hit and the attempt was a miss.
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    The distribution is skewed right with a tail that tapers off by 4 hits.
    The 'typical' (median) streak is 0 and this is where the peak of the distribution is.  
    The longest streak was 4.
    
    
```{r}
kobemean <- mean( kobe_streak )
kobemedian <- median( kobe_streak )
kobemax <- max( kobe_streak )
cat( 'The average (typical) streak length is', kobemedian,
     '\nThe longest streak was', kobemax)
```
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    ...
    This simulation yielded only 17 heads
...
4.  What change needs to be made to the `sample` function so that it reflects a 
    ...
    The following changes to the arguments given in the call to the sample() function we made:
    
    
```{r}
#size = 133 to simulate 133 shots on net
#prob = c( P(hit), P(miss)) -> c( 0.45, 1-0.45 ) -> c( 0.45, 0.55 )
set.seed(1980)
...
-   Describe the distribution of streak lengths. What is the typical streak 
    ...
    As with the data for Kobe, this distribution is skewed right with a tail and has a peak at 0 hits.
    The 'typical' (median) streak is 0.  
    The longest streak was 5.
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    I would expect the streak distribution to be somewhat similar: skewed with a right tail. The 'typical' streak should be the same (0), but there might be a different longest streak length and some other variations in the magnitudes of the streak lengths due to the natural deviations of the random process.
    However, why not just simulate the data again and find out?
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    The distributions look very similar. Therefore, there is not much evidence for Kobe Bryant to have had a hot hand. However, this doesn't make him any less of a legend. 

![RIP Kobe](https://media.nbclosangeles.com/2019/09/GettyImages-1196866980.jpg?fit=4461%2C3084)
